...
export function useChartData(config: ChartDataConfig) {
  ...
  const generateMockCandles = useCallback(
    (symbol: string, timeframe: TimeFrame, count: number): MarketCandle[] => {
  ...
  const calculateIndicators = useCallback(
    (candles: MarketCandle[], indicatorTypes: string[]): Record<string, (number | null)[]> => {
  ...
  const updateCandleFromTick = useCallback(
    (tick: { price: number; volume: number; timestamp: number }, timeframe: TimeFrame) => {
      const intervalMs = TimeFrameConverter.getMilliseconds(timeframe);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const indicators = config.indicators ? calculateIndicators(candles, config.indicators) : {};
