...
export function TradingViewChart({
  ...
  const [config, setConfig] = useState<ChartConfig>({
    type: 'candlestick',
    timeframe: '1h',
    indicators: new Map([['volume', { type: 'VOLUME', enabled: true }]]),
    drawings: [],
    showVolume: true,
    showGrid: true,
    showCrosshair: true,
  ...
  useEffect(() => {
    ...
    const invertPriceScale = (y: number) => {
      ...
      if (config.logScale) {
        return Math.exp(
          Math.log(minPrice) + normalized * (Math.log(maxPrice) - Math.log(minPrice)),
        );
      }
  ...
  }, [processedData, dimensions, config.logScale]);

  // Drawing functions
  const drawCandle = useCallback(
    (ctx: CanvasRenderingContext2D, candle: MarketCandle, scale: ChartScale) => {
      ...
      const candleWidth = Math.max(
        1,
        Math.min(10, (dimensions.plotWidth / processedData.length) * 0.8),
      );
  ...
  const drawLine = useCallback(
    (ctx: CanvasRenderingContext2D, data: MarketCandle[], scale: ChartScale) => {
  ...
  const drawGrid = useCallback(
    (ctx: CanvasRenderingContext2D, scale: ChartScale) => {
        ...
        switch (drawing.type) {
          ...
          case 'horizontal': {
            if (drawing.points.length >= 1) {
              ...
              ctx.textAlign = 'right';
              ctx.fillText(
                drawing.points[0].y.toFixed(2),
                dimensions.width - dimensions.margin.right - 5,
                y - 5,
              );
            }
            break;
  ...
  return (
    <div
      ref={containerRef}
      className={cn('relative bg-white dark:bg-gray-900 rounded-lg', className)}
    >
      {/* Toolbar */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={cn(
                'px-2 py-1 text-xs rounded transition-colors',
                config.showGrid ? 'bg-gray-700 text-white' : 'bg-gray-200 dark:bg-gray-700',
              )}
            >
              그리드
            </button>
            ...
            <button
              ...
              className={cn(
                'px-2 py-1 text-xs rounded transition-colors',
                config.showVolume ? 'bg-gray-700 text-white' : 'bg-gray-200 dark:bg-gray-700',
              )}
            >
              거래량
            </button>
            ...
            <button
              ...
              className={cn(
                'px-2 py-1 text-xs rounded transition-colors',
                config.logScale ? 'bg-gray-700 text-white' : 'bg-gray-200 dark:bg-gray-700',
              )}
            >
              로그
            </button>
          ...
          <div
            className={cn(
              ...
              processedData[processedData.length - 1].close >=
                processedData[processedData.length - 1].open
                ? 'text-green-500'
                : 'text-red-500',
            )}
          >
