...
async function updateHolding(portfolioId: string, transaction: any) {
  ...
  if (transaction.type === 'BUY') {
    if (existingHolding) {
      ...
      const newTotalCost = (currentQuantity * currentAvgPrice) + (transaction.quantity * transaction.price)
    ...
    if (newQuantity <= 0) {
      ...
      const realizedPnL = transaction.quantity * (transaction.price - Number(existingHolding.averagePrice))
