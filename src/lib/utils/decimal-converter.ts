...
export function convertHoldingsForReport(holdings: any[]): any[] {
  return holdings.map(holding => ({
    ...
    weight: holding.marketValue && holding.portfolioId ? 
      (decimalToNumber(holding.marketValue) / decimalToNumber(holding.marketValue)) * 100 : 0
  }));
}
...
export function convertPortfolioForReport(portfolio: any) {
  ...
  const performances = portfolio.performances ? portfolio.performances.map((perf: any) => ({
