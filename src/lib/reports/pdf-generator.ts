...
export async function generatePDFReport(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Header
      doc.fontSize(24).font('Helvetica-Bold').text('FLUX AI Capital', 50, 50);

      doc.fontSize(18).font('Helvetica').text(template.title, 50, 85);
      ...
      // Executive Summary
      doc.fontSize(16).font('Helvetica-Bold').text('Executive Summary', 50, yPosition);

      yPosition += 30;

      template.sections.forEach((section) => {
        // Section title
        doc.fontSize(14).font('Helvetica-Bold').text(section.title, 50, yPosition);

        yPosition += 25;

        // Section content
        doc.fontSize(11).font('Helvetica').text(section.content, 50, yPosition, { width: 500 });
      ...
      if (portfolio.holdings.length > 0) {
        doc.fontSize(14).font('Helvetica-Bold').text('Portfolio Holdings', 50, yPosition);
        ...
        portfolio.holdings.forEach((holding) => {
          ...
          doc
            .text(holding.symbol, 50, yPosition)
      ...
      if (portfolio.performances.length > 0) {
        yPosition += 30;
        doc.fontSize(14).font('Helvetica-Bold').text('Recent Performance', 50, yPosition);
